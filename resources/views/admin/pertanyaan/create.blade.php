            ...
            <div class="card-body p-4">
                <form action="{{ route('admin.pertanyaan.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kode') is-invalid @enderror" id="kode" name="kode" value="{{ old('kode') }}" required placeholder="Contoh: P01">
                        @error('kode')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('pertanyaan') is-invalid @enderror" id="pertanyaan" name="pertanyaan" rows="3" required placeholder="Tuliskan isi pertanyaan..."></textarea>
                        @error('pertanyaan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('tipe') is-invalid @enderror" id="tipe" name="tipe" required>
                                <option value="radio" {{ old('tipe') == 'radio' ? 'selected' : '' }}>Radio (Single Choice)</option>
                                <option value="checkbox" {{ old('tipe') == 'checkbox' ? 'selected' : '' }}>Checkbox (Multiple Choice)</option>
                            </select>
                            @error('tipe')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            @foreach($kategoris as $kat)
                                <div class="col-sm-6">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="kategori_ids[]" value="{{ $kat->id }}" id="kat_{{ $kat->id }}">
